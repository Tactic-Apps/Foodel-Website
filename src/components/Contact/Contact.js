...
class Contact extends Component {
  ...
  render() {
    ...
    if (!this.state.formSubmitted) {
      form = <form className="contact-form">{inputList}</form>;
    } else if (
      this.state.formSubmitted &&
      this.state.contactForm.submit.spinner === true
    ) {
      form = <div className="loader">Loading...</div>;
    } else if (
      !this.state.formSubmissionError &&
      this.state.contactForm.submit.spinner === false
    ) {
      form = (
        <div className="form-submission-message">
          <ion-icon name="checkmark" class="icon-small"></ion-icon>Thank you for
          contacting us. We'll get back to you as soon as possible.
        </div>
      );
    } else {
      form = (
        <div className="form-submission-message">
          <div className="form-submission-message">
            <ion-icon name="close" class="icon-small-fail"></ion-icon>Something
            went wrong with your form submission.
          </div>
          <button
            className="form-submission-message btn btn-full"
    ...
    return (
      <section className="section-form">
        <div className="map-box">
          <img src={map} alt="Jacksonville Map" style={{ height: "560px" }} />
        </div>
